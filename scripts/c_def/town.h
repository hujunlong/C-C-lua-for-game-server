...
enum TownType
{
	...
	kGetTownItem = kTownTypeBegin + 24,
	kTownTypeEnd = 10200,
...
struct FusionUpgradeResult //s2c
{
	...
	Result result;
	uint32_t is_success;//1：成功 
};
typedef struct FusionUpgradeResult FusionUpgradeResult;

struct GetTownItem //返回下面的几种建筑类型的状态
{
	static const Type kType = kGetTownItem;
	TownItemID id;
};
typedef struct GetTownItem GetTownItem;
